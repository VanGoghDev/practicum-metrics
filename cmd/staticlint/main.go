// Модель staticlint содержит кастомный линтер, который собран из нескольких линтеров, описанных ниже
// Стандартных статических анализаторов пакета:
// printf 		Вefines an Analyzer that checks consistency of Printf format strings and arguments.
...
//
// Анализаторов из opensource:
// errcheck 	errcheck is a program for checking for unchecked errors in Go code.
// mnd 			A vet analyzer to detect magic numbers.
//
// Самописные анализаторы:
// osexitcheck  check for os.exit in main func
//
// Анализаторов из staticcheck:
...
//
// Анализаторы запускаются автоматически. Чтобы запустить какой-то конкретный анализатор необходимо указать флаг -NAME с именем флага который хотите запустить.
//
//nolint:all // документация
package staticlint
